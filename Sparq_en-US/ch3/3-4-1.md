...
Our two variables `name_` and `value_` are respectively declared as a `SafeString` and `SafeUint256_t`.
...
TODO: functions that deal with tuples
...
Notice we can use primitive types and returns on functions just fine (this includes constructors, as seen below), as the contract's inner variables already ensure commit/revert safety due to their types being inherited from their SafeVariable counterparts.
...
Aside from our `name` and `value` variables, both constructors also take a few other arguments required by the base DynamicContract constructor:
...
* **(Optional)** A tuple of `EventParam` objects representing the event's arguments, where:
    * The first element is the argument type (e.g. `name` is a `std::string`, `value` is a `uint256_t`)
...
TODO: register functions for tuples
...
TODO: register events for tuples
