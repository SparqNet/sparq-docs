...
Using the Docker image is the recommended way to develop on the SDK. It will ensure that you have the correct environment to build and deploy the network.

After forking the project, you can now clone your forked repository and start developing on it, using the following commands:
...
After cloning, you can now build and run the Docker image. To do this, you need to install Docker on your system. You can find the installation instructions for your system on the links below:

* [Docker for Windows](https://docs.docker.com/docker-for-windows/install/)
* [Docker for Mac](https://docs.docker.com/docker-for-mac/install/)
* [Docker for Linux](https://docs.docker.com/desktop/install/linux-install/)

After installing Docker, you need to build the image. To do this, run the following command on the root of the repository, where the `Dockerfile` is located (we already at the root of the repository if you followed the steps above):

* For Linux/Mac: `sudo docker build -t orbitersdk-cpp-dev:latest .`
* For Windows: `docker build -t orbitersdk-cpp-dev:latest .`
...
To develop on the container with VSCode, you need to install the [Docker extension](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-docker) and configure it to use the container. After installing it, there is a `docker-compose.yml` file on the root of the repository that you can use to build and run the container. The only thing that you need to do is to change the `volumes` section to point to your local SDK folder:
...
After editing the `docker-compose.yml` file, you can right-click on the `docker-compose.yml` file and select `Compose Up` to build and run the container. After that, your container will be running and you can start developing on it. Click on the Docker extension icon on the left side of the VSCode window and you will see the container running. After this, you can right-click on the container and select `Attach Shell` to open a terminal on the container.

  ![VSCodeDockerExtension](img/VSCodeDockerExtension.gif)

Now you can develop on the container and build and deploy the network with the scripts provided on the `scripts` folder. After attaching the shell, you will be the root user on the container, so after attaching the shell, you can run the following commands to build and deploy the network:
