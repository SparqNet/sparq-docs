...
After editing the `docker-compose.yml` file, you can right-click on the `docker-compose.yml` file and select `Compose Up` to build and run the container. After that, your container will be running and you can start developing on it. Click on the Docker extension icon on the left side of the VSCode window and you will see the container running. After this, you can right-click on the container and select `Attach Shell` to open a terminal on the container. Now you can develop on the container and build and deploy the network with the scripts provided on the `scripts` folder.
