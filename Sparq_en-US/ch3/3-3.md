...
To deploy the local testnet, `cd` back to the project's root folder and run `./scripts/AIO-setup.sh`. The script will setup a local testnet with the [following information](https://github.com/SparqNet/orbitersdk-cpp#setting-up-a-local-network). TODO: shouldn't this info be here?
...
Running the script again will stop the testnet, rebuild it, replace it and restart it on the spot. If you wish to manually stop the testnet for some reason, call `tmux kill-server` (as we use tmux to properly deploy it). You can also read the script to find out the specific names of the tmux sessions.
