...
One last thing we have to do in the header is implement the `registerContract()` function. This function will register the contract's metadata and variables. To do that, include your contract's header in `customcontracts.h` (for this example, `#include "simplecontract.h"`)
