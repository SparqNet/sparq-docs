...
Then, open the file `src/contract/simplecontract.h` and add the following lines:
...
This is a simple skeleton so we can start building the proper contract. Notice we use [include guards](https://en.wikipedia.org/wiki/Include_guard) as a safety measure, we include and inherit the `DynamicContract` class, and two Safe Variable classes for the contract's inner variables - `SafeString` and `SafeUint256_t`.
...
Aside from that, the contract's name (passed to the base class constructor as `contractName`) MUST be EXACTLY the same as the name of your contract class. This is because `contractName` is used to load the contract type from the database, thus incorrectly naming it will result in a segfault at load time.
...
The destructor is responsible for saving the variables of the contract to the database, so that they can be loaded later by the second constructor, when `ContractManager` is being constructed.
...
After that, we must implement the `registerContractFunctions()` function, which is responsible for registering the contract's functions so they can be called later by a transaction or a RPC `eth_call`.
...
* ...taking only the first 4 (8 hex characters) bytes of the hash (also called the "functor").
...
In [`src/contract/contractmanager.h`](https://github.com/itamarcps/orbitersdk-cpp/blob/contract-development/src/contract/contractmanager.h), we must add two private functions within the ContractManager class that take `ethCallInfo` as argument:
...
In [`src/contract/contractmanager.cpp`](https://github.com/itamarcps/orbitersdk-cpp/blob/contract-development/src/contract/contractmanager.cpp), we must `#include "contractHeaderFile.h"` (in our case, `simplecontract.h`) and the previous declared functions.
