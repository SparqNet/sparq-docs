...
* We also include the reflection interface from Utils (`contractreflectioninterface.h`) which will let us properly register our contract
* And we create our `SimpleContract` class, inherit it from `DynamicContract`, and leave some space for the private and public members that will be coded next

### Step 2.1 - Declaring the Contract Variables and Functions

Now we can declare the variables of the contract and their respective functions in the header. We have to pay attention to some rules described earlier in [3.2](3-2.md):
...
One last thing we have to do in the header is implement the `registerContract()` function. This function will register the contract's metadata and variables. To do that, include your contract's header in `customcontract.h` (for this example, `#include "simplecontract.h"`)
...
Then, back in your contract's header, implement `registerContract()` like this:
...
* The following arguments are all the *types* of arguments inside its first constructor (from scratch) - you can copy-paste the constructor's arguments as-is and take out the names
...
We have a tool that is compiled alongside the project that generates the ABI of your contract. To use it, we have to go to `src/main-contract-abi.cpp` and add our contract's class name inside the chevrons in `writeContractsToJson<...>()`:
...
View functions MUST be `const` and return a `std::string` with ABI encoded data. In this case, the return values are the variables of the contract, so we can use the `ABI::Encoder` class to encode the variables and return the raw data. See [3-7](3-7.md) for more details on ABI.
...
Functions can be automatically registered using the reflection interface we included earlier in Step 2 (`#include "../utils/contractreflectioninterface.h"`). The register function should look like this:
...
We are done! All that's left is deploying the blockchain, and to do so, we must compile and deploy it by calling `scripts/AIO-setup.sh`. See [3.3](3-3.md) for more information.
