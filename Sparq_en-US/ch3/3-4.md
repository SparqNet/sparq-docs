...
Let's create a simple Solidity contract that allows two private variables to be changed by the owner of the contract. We will call this contract `SimpleContract`. You might notice it already exists within the project if you just forked it - in any case you can use this implementation as a reference, or you can delete it entirely and follow along from scratch if you want.
...
The contract has a pretty simple structure - a constructor that sets its owner, two functions that allow the owner to change the `name` and `value` variables, and their respective events for when that happens.
...
- Non-view functions can be either `const` or non-`const`, and can return either `void` or an ABI-supported type directly, if necessary
...
### Step 2.3 - Declaring the Contract Events

If your contract has events, you must declare them as `protected` and `void`, like this:
...
Where their blocks will simply call `this->emitEvent()`, an internal function available to all Dynamic Contracts. This function registers the emission of the event and requires at most three arguments:

* The event name (you can just pass `__func__` which is the same as `this->emitEvent("nameChanged", ...)`)
* **(Optional)** A list of event arguments in pairs, where the first element of the pair is the argument itself (e.g. `name`) and the second element is a bool that indicates whether that argument should be indexed or not. If your event has no arguments at all you can omit it or pass an empty list instead (e.g. `this->emitEvent(__func__, {})`)
* **(Optional)** A flag that indicates whether the event is anonymous or not (lacks an indexed keccak hash of its own signature). Events are non-anonymous by default, so if you wish you can omit this (which is the case for our example, it is equivalent to `this->emitEvent(__func__, { {name, true} }, false)` - if it was an anonymous event, the last flag would be `true` instead)

### Step 2.4 - Registering the Contract Class
...
* The first argument is a string vector that is a list of all the exact names of the arguments in the constructor, each one separated by a comma - in this case, `"name_"` and `"value_"`
...
* The following arguments are tuples, one for each function from the contract, that contain respectively:
  ...
  * A string vector that is a list of arguments that the function takes, if any (or a blank list if none)
...
If your contract has events, you should also register them - even though that's not exactly required, as events can be declared and emitted just fine without any kind of registration. *But*, as you'll probably need their ABI, as stated in Step 2.5, it's highly recommended that you register your events so their ABI can be generated as well.

Events are registered the same way as the contract class and its functions, but using a separate function called `ContractReflectionInterface::registerContractEvents()`:
...
Where the function requires your contract's name in the template (in this case, `registerContractEvents<SimpleContract>()`), and a tuple for each event in your contract, containing their name, the anonymous flag, and a list of tuples containing each of their arguments, with their names, types and indexed flags.

**Keep in mind that argument types MUST be named as their Solidity type names, NOT C++ ones**. So "name" in "nameChanged" is "string", NOT "std::string", and "value" in "valueChanged" is "uint256", NOT "uint256_t". This is important because those types will be checked internally to ensure they're correctly bound to their C++ counterpart.
...
With the header, declarations and (most of the) registering done, now we can proceed to the implementation itself. Open the source file (`simplecontract.cpp`) and `#include "simplecontract.h"` right at the beginning.
...
* `registerContractFunctions()` must be called at the end of both constructors
...
As stated earlier in Step 2.1, view functions must be `const` and return one of the ABI-supported types. In this case, the return values of our getters are the variables of the contract, so we can return the inner data from the SafeVariable by calling the `get()` function.
...
As stated earlier, non-view functions don't have any formal restriction like view functions do - they can be `const` or non-`const`, and can return `void` if no return is necessary, or one of the ABI-supported types if we want to return a specific value for some reason. In this case, as we're only setting a value which *does* modify the object, by C++ conventions, our setters are defined as `void` and non-`const`. We must also check that whoever is calling those functions is the actual creator of the contract (as it's coded in the original Solidity code reference at the start of this subchapter).
...
If your contract has events, you can emit them by simply calling them like they were any other function (which they actually are if you think about it!). The only thing you have to pay attention to is that *events can ONLY be emitted from non-view functions*, due to how const correctness works in C++ (pure/view functions are `const`, so trying to emit an event from one of them will result in a compilation error).
...
When registering manually, call info should be properly ABI-encoded/decoded. We provide an [ABI namespace](3-7.md), which contains an encoder and decoder that you can use to encode and/or decode Solidity's ABI strings in order to call a function.
...
void SimpleContract::registerContractFunctions() {
  ...
  this->registerFunction(Utils::sha3("setName(string)").get().substr(0,4), [this](const ethCallInfo &callInfo) {
    auto dec = ABI::Decoder::decodeData<std::string>(std::get<6>(callInfo));
    return this->setName(std::get<0>(dec));
  });
  this->registerFunction(Utils::sha3("setValue(uint256)").get().substr(0,4), [this](const ethCallInfo &callInfo) {
    auto dec = ABI::Decoder::decodeData<uint256_t>(std::get<6>(callInfo));
    return this->setValue(std::get<0>(dec));
  });
}
```
