...
First, in `contractmanager.h` let's declare two new *private* functions within `ContractManager`, both taking n `ethCallInfo` as an argument. The first function should create a new instance of the contract, and the second function should verify if the call to create the contract is valid.
