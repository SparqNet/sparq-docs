...
Inside `erc20wrapper.h`, let's implement the header (comments were taken out for easier reading):
...
ERC20Wrapper::~ERC20Wrapper() {
  ...
  for (auto it = this->_tokensAndBalances.cbegin(); it != this->_tokensAndBalances.cend(); it++) {
...
uint256_t ERC20Wrapper::getUserBalance(const Address& token, const Address& user) const {
  ...
  if (it == this->_tokensAndBalances.end()) return 0;
  auto itUser = it->second.find(user);
  if (itUser == it->second.end()) return 0;
...
* `src/contract/nativewrapper.h`
* `src/contract/nativewrapper.cpp`
