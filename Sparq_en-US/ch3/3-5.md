...
First of all, we need to create the header and source files (`erc20wrapper.h` and `erc20wrapper.cpp`, as stated above) and add them to `CMakeLists.txt`:
...
Inside `erc20wrapper.h`, let's implement the header (comments were taken out so it's lighter to read):
...
Like in [Chapter 3.4](3-4.md), you must include your contract's header in `customcontracts.h` to register it, and check it's set to generate its ABI through `main-contract-abi.cpp`. In this specific case for `ERC20Wrapper`, it's assumed that both steps are already done, but it's good to check again just in case.
...
The first constructor will create a new contract from scratch, as there is no previous existing contract to load. Because of that, you are required to initialize *all* the variables of your contract by hand within the `DynamicContract` constructor. Notice that your contract's name - "ERC20Wrapper" - is the same as your contract's class name - `ERC20Wrapper` - this match is *mandatory*, otherwise a segfault will happen.

The second constructor will load the contract from the database, and you are required to load all of its variables from there by hand as well. `this->getNewPrefix()` does the same as `this->getDBPrefix()` from the previous chapter, but with a custom string appended to it, so this would be equivalent to `DBPrefix::contracts` + the contract's address + `_tokensAndBalances`.

Both constructors have to call `registerContractFunctions()`. This will be explained further. The destructor is responsible for saving the current information within the contract back to the database.

### Step 4 - Implementing the contract functions

This step is pretty straightforward, as we just follow the rules explained in Step 2.1 of [Chapter 3.2](3-2.md):
...
Notice that, in the example above, some functions are calling functions from another contract. This is done by calling `this->callContractViewFunction()` (*for view functions*) and `this->callContractFunction()`(*for non-view/callable functions*), both of which require the following arguments:
...
* The function's arguments, if there's any - in this case:
  * `ERC20::balanceOf()` will receive our contract's own address as `this->getContractAddress()`
  * `ERC20::transfer()` will receive the receiver address as `to` or `this->getCaller()`, and the value to be transferred as `value`
  * `ERC20::transferFrom()` will receive the sender's address as `this->getCaller()`, the receiver's address as `this->getContractAddress()`, and the value to be transferred as `value`

Alternatively, *for view functions specifically*, you can get a pointer to the other contract with `this->getContract()` and call the function directly from it, passing along any required arguments. The `getContract()` function itself is automatically protected in the case of casting a wrong typed contract or calling an inexistent contract. So the implementation of `getContractBalance()` could also be done like this:
...
As stated in Step 2.3 of [Chapter 3.4](3-4.md), we can do this *automatically* or *manually*. Here's how each method would look like:
...
void ERC20Wrapper::registerContractFunctions() {
  this->registerViewFunction(Hex::toBytes("0x43ab265f"), [this](const ethCallInfo &callInfo) {
    auto dec = ABI::Decoder::decodeData<Address>(std::get<6>(callInfo));
    return this->getContractBalance(std::get<0>(dec));
  });
  this->registerViewFunction(Hex::toBytes("0x6805d6ad"), [this](const ethCallInfo &callInfo) {
    auto dec = ABI::Decoder::decodeData<Address, Address>(std::get<6>(callInfo));
    return this->getUserBalance(std::get<0>(dec), std::get<1>(dec));
  });
  this->registerFunction(Hex::toBytes("0xf3fef3a3"), [this](const ethCallInfo &callInfo) {
    auto dec = ABI::Decoder::decodeData<Address, uint256_t>(std::get<6>(callInfo));
    this->withdraw(std::get<0>(dec), std::get<1>(dec));
  });
  this->registerFunction(Hex::toBytes("0xa5f2a152"), [this](const ethCallInfo &callInfo) {
    auto dec = ABI::Decoder::decodeData<Address, Address, uint256_t>(std::get<6>(callInfo));
    this->transferTo(std::get<0>(dec), std::get<1>(dec), std::get<2>(dec));
  });
  this->registerFunction(Hex::toBytes("0x47e7ef24"), [this](const ethCallInfo &callInfo) {
    auto dec = ABI::Decoder::decodeData<Address, uint256_t>(std::get<6>(callInfo));
    this->deposit(std::get<0>(dec), std::get<1>(dec));
  });
}
```
...
Finally, go back to the project's root, run `./scripts/AIO-setup.sh` to setup your local network, deploy your contract using the chain owner's private key and test your contract's compatibility with your favorite frontend tool.

It is also possible to create tests using the internal test framework (catch2) - see the following files for more information:

* `tests/contract/erc20wrapper.cpp`
* `tests/contract/nativewrapper.cpp`
* `src/contract/templates/nativewrapper.h`
* `src/contract/templates/nativewrapper.cpp`
* `src/contract/templates/erc20.h`
* `src/contract/templates/erc20.cpp`
