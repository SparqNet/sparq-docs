...
The `src/utils/contractreflectioninterface.h` file contains the **ContractReflectionInterface** namespace - utility functions for enabling [reflections](https://en.wikipedia.org/wiki/Reflective_programming) in C++, powered by [meta.hpp](https://github.com/blackmatov/meta.hpp), to help with writing native contracts in a quicker and easier way (e.g. registering functions and variables using less lines of code, as the reflections abstract away and take care of most details).
...
The `src/utils/hex.h` file contains the **Hex** class - an abstraction of a strictly hex-formatted string (meaning it only accepts the characters within the range of `0x[1-9][a-f][A-F]`), which can also be set to strict or not (which defines whether the string REQUIRES the `0x` prefix or not to be considered valid).
...
* Aliases for working with raw-byte strings (**Bytes, BytesEncoded, BytesArr, BytesArrView, BytesArrMutableView**) and call data (**ethCallInfo, ethCallInfoAllocated**)
