...
The `src/utils/hex.h` file contains the **Hex** class - an abstraction of a strictly hex-formatted string (meaning it only accepts the characters within the range of `0x[1-9][a-f][A-F]`), which can also be set to strict or not (which defines whether the string REQUIRES the `0x` prefix or not to be considered valid). Also contains aliases for working with raw-byte strings, such as **Byte, Bytes, BytesArr, BytesArrView and BytesArrMutableView**.
...
* Aliases for working with:
    * Raw-byte strings (**Byte, Bytes, BytesArr, BytesArrView, BytesArrMutableView**), as well as helper functions for converting and/or manipulating them (e.g. `appendBytes()`)
