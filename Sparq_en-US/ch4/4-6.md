...
Before diving into this chapter, it is recommended to read Chapter 3 - Contracts.

Contracts within OrbiterSDK differ significantly from those in EVM blockchains. While we provide the same functionality as Solidity within DynamicContract, Protocol Contracts break away from the "rules" of VM contracts, allowing for more flexibility and power.
...
In OrbiterSDK, contract access within a transaction (e.g., when processing a new block) or through RPC eth_call/eth_estimateGas methods is managed by the ContractManager. The ContractManager is a protocol contract that is automatically created when the blockchain is initiated. It is responsible for managing transaction/RPC access to all contracts within the blockchain, as well as the creation and object ownership of DynamicContract.

The ContractManager does not own or create any Protocol Contracts. Instead, protocol contracts are created during blockchain initialization, and a reference to each contract is stored within the ContractManager, allowing it to access them.
...
Every contract within OrbiterSDK indirectly inherits from the ContractGlobals class. This class provides contracts with global variables accessible through transaction/RPC calls, and these variables are set by the State when calling the contract.
...
The ContractGlobals class does not have specific constructors or destructors.

## The ContractLocals Class

Every contract within OrbiterSDK indirectly inherits from the ContractLocals class. This class provides contracts with local variables accessible through transaction/RPC calls, and these variables are set by the ContractManager when calling a contract.
...
The ContractLocals class does not have specific constructors or destructors.
...
Address, Owner, ChainId, and Name are stored in the database following the prefix: DBPrefix::contracts + contractAddress + variable name.

BaseContract has two constructors: one for creating a new contract and another for loading a contract from the database.

Protocol Contracts are only initialized when the blockchain is created, so only the "create" constructor is used. There is no need to use the "Load from DB" constructor, as the BaseContract variables of the contract will always remain the same. Meanwhile, DynamicContract can be either created or loaded from the database.
...
DynamicContract are contracts that automatically provide protection against reverts in the case of a throw within a specific function call. In C++, when you call a function that changes a variable and then throws an exception later, the changed variable will not be reverted. Consider the following example:
...
if you call ```updateValueAndThrow``` even inside a try/catch block, the value of ```myMap[key]``` will be changed, but the exception will be thrown. However, this is not the case with DynamicContract, which automatically revert changes made to the contract due to their built-in protection mechanisms.

This functionality is achieved by using variables that inherit from the SafeBase class. A SafeBase variable adheres to the following rules:
...
- All SafeBase variables of the contract must be declared in the contract's header file and initialized with this as the parameter. This enables the SafeBase variable to mark itself as used in the ContractManager, allowing proper reversion of changes made to the contract.
- All SafeBase functions must call check() and return the temporary value (or reference to the temporary value).
- All non-const SafeBase variables must call markAsUsed() when accessed, allowing the DynamicContract to properly register the variable as used within the function call and revert or commit changes made to the contract accordingly. 

DynamicContract includes a vector of references to SafeBase variables, which is used to register used SafeBase variables within a specific function call (more on this later).
...
We provide three different function types for DynamicContract: non-payable, payable, and view functions. Non-payable and payable functions must return void, while view functions must return the respective ABI-encoded return value.
...
DynamicContract must register functions that are callable by a transaction or an RPC request by overriding the registerContractFunctions() method and calling it during the constructor. DynamicContract provides three different functions for registering functions, one each for non-payable calls, payable calls, and HTTP JSON RPC requests.
...
The functor argument should follow Solidity function signature conventions. For example, an ERC20 transfer function would be registered as keccak256(transfer(address,uint256)).substr(0,4). The f argument is a lambda function that will be called when the function is called. The ethCallInfo argument is a tuple containing the following information:
...
When registering a function, the DynamicContract class will store the respective signature and function in an unordered_map, allowing a given transaction to call any function within that contract.
...
We provide two access functions for calling a function:
...
You can see that within the ethCall function, we call the respective function based on the data within that call. Additionally, we call updateState(bool) depending on the result of the function call and whether it is a simulation or not.

The updateState() function will iterate through the marked SafeBase variables and call commit()/revert() depending on the boolean argument

## The ContractManager class

The ContractManager class is responsible for managing access to all contracts and the ownership of DynamicContract within the blockchain. It is in charge of creating new instances of dynamic contracts and providing an interface for the State (Transactions) and HTTP (JSON RPC).

ContractManager is a Protocol contract, meaning that it is a contract whose object is created during the blockchain initialization. It is responsible for loading all the DynamicContract instances and managing their ownership, among other tasks.

## Accessing Contracts through ContractManager

ContractManager offers three types of access to the contract instances within the blockchain: one that takes transactions as inputs (for State when processing blocks), one that takes a fully constructed ethCallInfo to simulate a transaction (for RPC to answer eth_estimateGas to let the user know if the transaction is going to revert or not), and one that takes an eth_call request as input. The definition for transactions is as follows:
...
As noted, all ProtocolContracts must be manually handled by the ContractManager. This is because ProtocolContracts are not registered as DynamicContract but rather as a class within the ContractManager. This is done because ProtocolContracts are not created by the ChainOwner calling ContractManager, but rather by the blockchain itself.

You can see that the ContractManager will set the ContractLocal variables (caller, origin, value, and commit) and call the ethCall function of the contract. The commit will let the Contract know if this is a simulation or not. After that, it will check if the function called was a payable function, and if it was, it will call processContractPayable() of the State class, which will update the balances of the accounts accordingly to the operations performed. Be aware that this State-related access by all contracts is protected (more on that in later sections).
...
Where the initialization of the contract is done as following:
...
The interface allows the Contract to interact with other contracts and the native token balance of other addresses on the chain, effectively accessing the State class to get the balance of an address. More information about it is located within the next section. The contractName is the name of the class, which is not argumented to the class directly; instead, the class constructor initializes it during the initialization list. It is used to identify the contract during loading. The address is derived using the deriveContractAddress() function, which is a function that derives the address of the contract using the address of the creator and the nonce of ContractManager (differentiating from EVM, where the account nonce is used). The creator is the address of the caller, the chainId is the chainId of the node, and the DB is the DB of the node.

In order to integrate the constructor for creating a new ERC20 contract, we need to add two private functions to the ContractManager: one for creating the contract and another one for validating if a given transaction can create that contract. The naming must follow createNewCONTRACT_NAMEContract and validateCreateNewCONTRACT_NAMEContract, where CONTRACT_NAME is the class name of your contract. For ERC20, we will need to add the following:
...
Notice that the function declaration has a commented Solidity function above it. This function must consider the arguments for your contract as if it were a Solidity contract. You can add this function to your ContractManager JSON ABI and call it with any web3 client to create a new instance of the contract. Nevertheless, let's delve deeper into how these functions are implemented.

The derivedContractAddress can be described using the following pseudo-code: sha3(rlp(tx.from() + manager.nonce()).substr(12);. Its implementation is as follows:
...
Effectively, the function for creating or validating the creation of a new contract must also check if the address is already in use (throwing an exception if it is). The validate function must only check if the creation is actually valid, whereas the create function should store a new contract instance within the this->contracts map.
...
The ContractManagerInterface class is a nested class within ContractManager. It is used to allow the contracts to interact with other contracts and to enable the contracts to modify balances. The definition of the class is as follows:
```cpp
...
The reason for using a interface instead of accessing directly the ContractManager or the State is to be able to do it in a isolated/secure way, for example, if we want to send tokens from a contract to another address, we don't access the balance directly from the State, every time a function is called by a **user** (not another contract), the balances map available for contracts is empty, only populating what is currently being accessed and not previously available, and when modifying the balance, only the mapping within ContractManager is modified, therefor allowing for multiple nested contract functions to revert and not affect the State neither other contracts.


There are two methods for Dynamic Contracts to access other Contracts on the blockchain, one for calling only view functions and another for calling functions that can affect the variables of the blockchain, the first one is done by casting the contract pointer from the inner ContractManager mapping to the desired contract type as ```const``` therefor only allowing view functions, the second one is done by casting the contract pointer from the inner ContractManager mapping to the desired contract type as ```non-const``` therefor allowing view and non-const functions.

We use ```this->interface.getContract<CONTRACT_TYPE>(address)``` within a contract function in order to access that contract if existing within that address, take note that if the type check fails, it will revert.

For calling a non-const contract function, it is another history because it is necessary to only do the call through ContractManager, otherwise SafeBase variables won't be affected by a revert.

This is done by calling ```this->interface.callContract(address, ethCallInfo, value_if_payable)``` within a contract function, this will call the function within that contract with the proper safety checks in place, it is necessary the properly encode the function signature and arguments within the ethCallInfo following Solidity rules, the implementation for ```callContract``` is as following:
...
Every time sendToken or getBalanceFromAddress is called, it calls populateAddress to grab the balance from the State and populate within the ContractManager, not only that it also verifies if the contract has enough balance to actually send the tokens, if not, it will revert, the implementation for functions that access balances is as following:
...
## Not breaking your blockchain - Using ProtocolContract

Protocol contracts do not offer the same level of ease of use and security as DynamicContracts; however, by sacrificing some of these features, they enable more complex functionalities that would be impossible to achieve with DynamicContracts alone. For instance, protocol contracts can call functions without an active transaction call when processing a block, access files within the system, and make requests to other nodes.
...
By adhering to these best practices, you can create Protocol Contract that maintain consistent behavior across nodes, ensuring the stability and security of your blockchain network. Remember that the key to developing robust ProtocolContracts lies in managing the context and ensuring that all nodes in the network can process information in the same way.